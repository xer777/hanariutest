		...
		for ($i = 0; $i < $len; $i++)
		{
			...
			if ($m_state == 0)
			{
				...
				else
				{
					...
					\trigger_error('\Core\UTF8::to_unicode: Illegal sequence identifier in UTF-8 at byte '.$i, E_USER_WARNING);
					return FALSE;
				}
			}
			else
			{
				...
				if (0x80 == (0xC0 & $in))
				{
					...
					if (0 == --$m_state)
					{
						...
						if (((2 == $m_bytes) AND ($m_ucs4 < 0x0080)) OR
							...
							($m_ucs4 > 0x10FFFF))
						{
							\trigger_error('\Core\UTF8::to_unicode: Illegal sequence or codepoint in UTF-8 at byte '.$i, E_USER_WARNING);
							return FALSE;
						}
				...
				else
				{
					...
					throw new \Core_Exception("\Core\UTF8::to_unicode: Incomplete multi-octet sequence in UTF-8 at byte ':byte'", array(
						':byte' => $i,
					));
				}
			}
